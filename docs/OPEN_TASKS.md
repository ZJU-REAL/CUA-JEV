...
An unrecorded live three-chapter run with `dashscope/qwen-plus-latest` and Jev passed the visit, CLI, MCP, citation, and file gates in **11 external actions / 147.4 s**: 4 DOM clicks, 1 CLI version check, 5 MCP calls, and 1 file write. The five MCP calls included redundant reads, prompting the current-page-once mode above. A deterministic **20-action** regression covers nine simulated browser sources, real MCP stdio, CLI, and a real file artifact; its planner and browser are fixtures. An eight-chapter live run stopped after five actions when a transient browser snapshot had no DOM controls; that path now has a bounded readiness retry. The next run received repeated model-gateway `ReadTimeout` at step zero; minimal calls to two separate model IDs also timed out while `/models` remained reachable. The new per-visit mode has protocol and deterministic tests but **has not yet completed a live 20-step model+Jev run**. Private JSONL traces and output artifacts stay under ignored `runs/`, not in the public repository.
