...
For longer goals, the same host now accepts caller-supplied page-visit gates and a **create-only cited text artifact**. An optional same-origin MCP reader can be constrained to the current browser URL, once per page, and required to read each visited source. In an unrecorded live three-chapter pilot, model planning + Jev completed **11 verified external actions** across DOM, CLI, MCP, and file API and created a three-source note. Five MCP calls in that run included redundant reads; it took **147.4 s**, so it is not a speed claim. A deterministic 20-action regression test covers nine sources and all four channels, but uses a simulated browser and scripted planner—not a live unseen-goal result. The subsequent eight-chapter live attempt stopped after five actions on an empty transient DOM; a bounded DOM-ready retry was added. Its next retry stopped at step zero because the campus model gateway timed out even on a minimal request. **No successful live ~20-action case, repeated reliability study, or macOS validation is claimed yet.** Details and reproduction options are in [open-task notes](docs/OPEN_TASKS.md).
