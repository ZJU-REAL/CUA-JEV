...
可选的 [MCP 工具提供者](src/cua_jev/mcp_surface.py)也已进入同一候选空间，以 `m:` 引用与 DOM、UIA、CLI 等路线并列。由用户信任的本地配置固定 MCP stdio 程序、工具白名单及参数；模型只能选择已有引用，不能生成启动命令或改写参数。必须同时提供 `--mcp-profile` 与 `--allow-mcp-actions` 才能执行，且默认按可能有外部副作用的动作处理。真实 MCP 协议调用已在 DOM＋UIA 组合的端到端测试中通过；这**不代表任意第三方 MCP 服务安全可靠**。[配置格式与限制](docs/OPEN_TASKS.md#registered-mcp-calls)见详细说明。
...
- [ ] 在固定可信配置之外加入基于工具 schema 的 MCP 发现与参数生成；完善跨通道失败恢复、逐动作安全确认与长期回归基准。
