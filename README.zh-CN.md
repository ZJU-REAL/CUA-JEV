...
开放任务宿主现已支持浏览器访问验收、只创建新文件的引文产物，以及可选的“当前页只经 MCP 读取一次／每个来源都须读取”约束。一次未录制的真实三章联调由模型规划、Jev 决策，完成 **11 个外部执行动作**，覆盖 DOM、CLI、MCP、文件 API，并生成三来源笔记；其中有 5 次 MCP 调用存在重复读取，总耗时 **147.4 秒**，不能据此声称速度优势。另有一个基于模拟浏览器和脚本规划器的 20 动作回归测试，**不是真实陌生任务成绩**。八章真实尝试在第 5 步遇到瞬态空 DOM，已加入有限重试；下一次重跑则在第 0 步遭遇校园模型网关超时。目前尚未跑通并录制真实约 20 步案例，也没有 macOS 实测或重复成功率统计。
