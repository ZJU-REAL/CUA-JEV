...
现已增加[实验性的开放任务路径](docs/OPEN_TASKS.md)：低频模型规划器根据实时 DOM 或 Windows UI Automation 控件生成带引用的动作选项，Jev 再做类型化动作与执行通道选择。OpenAI 兼容协议适配器和一个短程真实浏览器任务已与 Jev 联调跑通，但**还不能声称支持任意任务**。
