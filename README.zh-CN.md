...
现已增加[实验性的开放任务路径](docs/OPEN_TASKS.md)：规划模型根据实时 DOM 或 Windows UI Automation 控件生成带引用的动作选项，Jev 再做类型化动作与执行通道选择。其中一个[跨软件试验视频](https://zjureal.com/CUA-JEV/#open-task)展示了模型从 Python 官网查找发行信息、写入来源明确的笔记并在 VS Code 打开，另有独立事实验收。该路径仍是受限的“浏览器→编辑器”任务族，**不能声称支持任意任务**；这次试验中模型与 Jev 各调用了三次，也尚未证明开放任务上的速度或成本优势。
